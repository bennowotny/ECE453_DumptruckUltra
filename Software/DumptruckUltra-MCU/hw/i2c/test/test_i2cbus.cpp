...
int main() {
    ...
    i2cBus.i2cWriteReg<1>(IO_EXP_ADDR, IO_EXP_POLREG, {0x00});

    i2cBus.i2cWriteReg<1>(IO_EXP_ADDR, IO_EXP_OUTREG, {0x01});
    ...
    while (true) {
        i2cBus.i2cReadReg<1>(IO_EXP_ADDR, IO_EXP_OUTREG, read_val);
        ...
        i2cBus.i2cWriteReg<1>(IO_EXP_ADDR, IO_EXP_OUTREG, {value});
        cyhal_system_delay_ms(500);
    }
}
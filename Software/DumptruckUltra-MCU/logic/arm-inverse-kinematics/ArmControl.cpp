...
ArmControl::ArmControl(ArmLayout &armLayout, std::function<bool()> isClawOpen) : shoulder{armLayout.shoulder},
                                                                                 elbow{armLayout.elbow},
                                                                                 wrist{armLayout.wrist},
                                                                                 claw{armLayout.claw},
                                                                                 sweepParams{} {
    shoulder.enable();
    elbow.enable();
...
void ArmControl::collect(float distanceForward_m, float distanceHorizontal_m) {
    ...
    float xForward_mm = distanceHorizontal_m;
    float yForward_mm = distanceForward_m + 30;
    // float xForward_mm = xForward_m * 1000.0;
    // float yForward_mm = yForward_m * 1000.0;
    float zForward_mm = -30.0; // constant z position

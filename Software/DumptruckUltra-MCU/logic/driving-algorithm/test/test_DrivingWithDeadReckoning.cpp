...
auto main() -> int {
    ...
    std::unique_ptr<Logic::DeadReckoning::DeadReckoning> deadReckoning{};
    ...
    deadReckoning = std::make_shared<DeadReckoning>([uu2{uut.get()}]() { return uu2->getPower(); });

    auto imu{std::make_unique<IMU>(
        i2cBus,
        [uu2{deadReckoning.get()}](const Hardware::IMU::AccelerometerData &msg) -> void { 
            printf("%f\n", msg.Ax);
            uu2->sendAccelerometerMessage(msg); },
    ...
    xTaskCreate(
        [](void *params) -> void {
            auto dr = static_cast<Logic::DrivingAlgorithm::DrivingAlgorithm *>(params);
            while (true) {
                if (dr->getStatus() == Logic::DrivingAlgorithm::DrivingAlgorithmStatus::COMPLETE) {
